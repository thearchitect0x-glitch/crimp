...
| 9 | **Drift and outage detection** | **EXISTS as of cap-09** (was PARTIAL) · informational, subject-free | `src/domain/insight.ts` computes rates over windows — source reliability, the quadrant, cliffs — and `sweepLag()` (`seal.ts`) is an outage signal for the correction channel. Missing: rolling rate per rule per reason code, a baseline + σ, the `drift` finding, and (again) a findings table. Note the volume-floor discipline in `insight.ts`: an inferential rate below the floor is null, a census count never is. Drift is inferential and should inherit the floor. |
| 10 | **Human decisions through the same gate** | **EXISTS as of cap-10** (was PARTIAL) | There is no separate human path and no free-text outcome anywhere: a caseworker seals through `/v1/seals` with an `operator`/`principal` key, and `sealed_by` records the authority (`seal.ts`). "Rule selection from the committed ruleset" needs **8**. "Log the actor as attester on every fact" is **MISSING** — `attestations` has `source` (a declared system) but no `attester` (the credential that asserted). One column. |
...
| **Findings table** — class, subject (seal / rule / tenant), detail, occurred_at | 3 · 6 · 9 | **Landed in cap-03.** `findings` + `finding_classes` reference table (migration 014); no subject id; nothing in the seal path reads it |
| **Attester on attestations** | property 1 · 10 | **Landed in cap-01.** `attestations.attester` + `seal_facts.attester`, populated from the key id, old rows null |
| **Per-program config** — clocks, restoration windows, appeal-rights text | 3 · 4 · 5 | Clocks in cap-03 (`clocks.config.ts`); appeal rights in cap-04 (`notice.config.ts`); restoration windows in cap-05 (`restoration.config.ts`) — every value `TODO(legal-confirm)` |
