...
| **Per-program config** — clocks, restoration windows, appeal-rights text | 3 · 4 · 5 | Clocks in cap-03 (`clocks.config.ts`); appeal rights in cap-04 (`notice.config.ts`, three programmes); restoration with cap 5 — every value `TODO(legal-confirm)` |
